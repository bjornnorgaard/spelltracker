...
<div class="space-y-4">
    <button onclick={handleBack} class="anchor"> ← Back to Characters</button>
    ...
    {:else}
        <div class="flex flex-col gap-4 justify-between">
            <div>
                ...
                <p class="opacity-75">Level {character.level} {character.class}</p>
            </div>
            <div class="flex gap-2 justify-end">
                <button onclick={handleEdit} class="btn preset-tonal">Edit</button>
                <button onclick={handleImportSpells} class="btn preset-tonal">Import Spells</button>
            </div>
        </div>
        ...
        <div class="card p-4 preset-tonal">
            <div class="flex justify-between items-center">
                ...
                <div class="flex gap-2">
                    ...
                    {/if}
                    <button onclick={handleEditSlots} class="btn preset-tonal text-sm"> Edit Slots </button>
                </div>
            </div>
            ...
            {#if Object.keys(character.spellSlots).length === 0}
                <div class="card p-8 text-center">
                    ...
                    <button onclick={handleEditSlots} class="btn preset-filled-primary-500"> Add Spell Slots </button>
                </div>
            {:else}
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {#each Object.entries(character.spellSlots) as [level, slot]}
                        <div class="card p-4">
                            ...
                            <!-- Controls -->
                            <div class="flex gap-2">
                                <button onclick={() => useSpellSlot(Number(level))} disabled={slot.used >= slot.total} class="btn preset-filled-primary-500 flex-1 text-sm" title="Use spell slot">
                                    Use
                                </button>
                                <button onclick={() => restoreSpellSlot(Number(level))} disabled={slot.used === 0} class="btn preset-tonal flex-1 text-sm" title="Restore spell slot"> Restore </button>
                            </div>
                        </div>
                    {/each}
                </div>
